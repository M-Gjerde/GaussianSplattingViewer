...
def create_look_at_from_colmap(tx, ty, tz, qw, qx, qy, qz):
    ...
    rot_matrix = quaternion_to_rotation_matrix(qw, qx, qy, qz).T

    # Flip Z-axis for handedness correction (if necessary)
    ...
    world_forward = apply_rotation(forward_vector, rot_matrix_flip)
    world_up = apply_rotation(up_vector, rot_matrix_flip)

    # The center point the camera looks at is obtained by adding the world forward vector to the camera position
    center_point = camera_pos + world_forward

    return camera_pos, center_point, world_up


def look_at(camera_pos, center_point, up_vector):
    # Direction from the camera to the center point
    forward = center_point - camera_pos
    ...
    mat[0, :3] = right
    mat[1, :3] = up
    mat[2, :3] = -forward  # Inverse direction for lookAt
    mat[:3, 3] = -np.dot(mat[:3, :3], camera_pos)
...
def apply_quat_to_vec(quat, vec):
    ...
    qvec = glm.vec3(quat.x, quat.y, quat.z)
    uv = glm.cross(qvec, vec)
...
def create_look_at_from_colmap_2(tx, ty, tz, qw, qx, qy, qz):
    ...
    # Forward vector in OpenGL camera space (looking down -Z axis)
    forward_vector = glm.vec3(0, 0, 1)
    world_forward = camera_position + apply_quat_to_vec(camera_orientation, forward_vector)

    # Up vector in OpenGL camera space
    up_vector = glm.vec3(0, 1, 0)
    world_up = apply_quat_to_vec(camera_orientation, up_vector)

    # Calculating the center point the camera is looking at
    center_point = camera_position + world_forward
...
def main(trained_model=None, colmap_poses=None):
    ...
    if trained_model is not None:
        ...
        gaussians.opacity = np.concatenate((gaussians.opacity, new_opacity), axis=0)
        gaussians.scale = np.concatenate((gaussians.scale, new_scale), axis=0)
        gaussians.rot = np.concatenate((gaussians.rot, new_rot), axis=0)
        gaussians.sh = np.concatenate((gaussians.sh, new_sh), axis=0)
        print("Finished loading gaussians")

    g_renderer.update_gaussian_data(gaussians)
    print("Sorting and updating")
    g_renderer.sort_and_update(g_camera, use_file, pose)
    ...
    while not glfw.window_should_close(window):
        ...
        if frame_counter == 5 and animate:
        ...
        if len(camera_poses) > 0 and (
                manual_save or saved_image[pose_index] is False) and not skip_frames or manual_save:
            print(
                f"Saving from Image ID: {camera_poses[pose_index][0]}. Rendered image name: {pose_index}.png. Original: {camera_poses[pose_index][-1]}")
            ######### LEFT FBO
            g_renderer.set_render_mod(g_render_mode - 3)
            gl.glBindFramebuffer(gl.GL_FRAMEBUFFER, fbo_center)
            gl.glClear(gl.GL_COLOR_BUFFER_BIT)
            ...
            disparity_map_scaled = (disparity_map * 65535).astype(np.uint16)
